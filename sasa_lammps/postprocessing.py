...
class SASA_postprossing:
    ...
    def neighbor_analysis(path, SASA_outfile, gro_file):
        ...
        pos=np.loadtxt(os.join(path,SASA_outfile), skiprows=2, usecols=(1,2,3))
        ...
        pipeline = import_file(os.join(path, gro_file))
    ...
    def atom_analysis(path, SASA_outfile, neighbor):
            # load SASA results
        spec = pd.read_csv(os.join(path, SASA_outfile), delim_whitespace=True, skiprows=1) \
                .drop(['atom'], axis=1)
        ...
        spec=spec.sort_values('eint/eV', ascending=True).drop_duplicates(subset=['ID']) \
        ...
        pd.DataFrame(result).to_csv(os.join(path, 'strongest_interaction_percent.txt'), 
                                    sep='\t', index=False)
        return result
    ...
    def atom_analysis_plot(path, neighbor, result):
        ...
        #save
        plt.savefig(os.join(path, 'attack_vs_atomtype.png'), dpi = 300, bbox_inches='tight')
        plt.show()
        
    ### Residue specific analysis ###
    def residuelist(path, gro_file):
        # read .gro file
        gro = pd.read_csv(os.join(path, gro_file), delim_whitespace=True, skiprows=2, 
        ...
        residuelist = pd.DataFrame([re.split(r'(\d+)', s) for s in gro['ResType']]) \
                .drop(columns = 0).drop_duplicates(subset=1)
        np.savetxt(os.join(path, 'residuelist.txt'), residuelist,fmt='%s', delimiter='  ', 
                    header='ResNum ResType', comments='')
        return residuelist

    def residue_analysis(path, SASA_outfile, residuelist):
        df = pd.read_csv(os.join(path, SASA_outfile), delim_whitespace=True, skiprows=1)
        res = pd.read_csv(os.join(path, 'residuelist.txt'), delim_whitespace=True,)
        ...
        emin.to_csv(os.join(path, 'minimum_energy_values.txt'), sep='\t', index=False)
        ...
        pd.DataFrame(result).to_csv(os.join(path,'interaction_probability.txt'), sep='\t', 
                index=False)
    ...
    def residue_analysis_plot(path, res_names, result):
        ...
        ax.tick_params(which='minor',width=1, length=6, right=True,)
        #save
        plt.savefig(os.join(path, 'attack_vs_residue.png'), dpi = 300, bbox_inches='tight')
        plt.show()
